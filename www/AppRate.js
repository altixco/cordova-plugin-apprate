...
AppRate = (function () {
  ...
  promptForStoreRatingWindowButtonClickHandler = function (buttonIndex) {
    var base = AppRate.preferences.callbacks, currentBtn = null;
    console.log('promptForStoreRatingWindowButtonClickHandler', 'buttonindex', buttonIndex);
  ...
  promptForFeedbackWindowButtonClickHandler = function (buttonIndex) {
    var base = AppRate.preferences.callbacks, currentBtn = null;
    console.log('promptForFeedbackWindowButtonClickHandler', 'buttonindex', buttonIndex);
    switch (buttonIndex) {
      case 1:
        currentBtn = localeObj.noButtonLabel;
        updateCounter('stop');
  ...
  showDialog = function (immediately) {
    ...
    if (counter.countdown === AppRate.preferences.usesUntilPrompt || immediately) {
      ...
      if (AppRate.preferences.simpleMode) {
        navigator.notification.confirm(localeObj.message, promptForStoreRatingWindowButtonClickHandler, localeObj.title, [localeObj.rateButtonLabel, localeObj.laterButtonLabel,localeObj.cancelButtonLabel,]);
      } else {
